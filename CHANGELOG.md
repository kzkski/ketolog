...
- **メニュー追加ドロワー**: バーコード読み取り ON 時はプレビュー周りをスクロール内で sticky にし、映像に最大高さを付けて小画面でも見えやすくした。新規追加時の下部3ボタンはスマホで2列グリッド＋短いラベルに整理した。
...
- メニュー追加ドロワーで**登録先のお店**を冒頭・登録ボタンで明示し、補足文言を「メニュー一覧」基準に統一。
