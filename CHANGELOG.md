...
- **プリセット**: ステーキチェーン公式栄養表に基づく外食プリセット **`external-steak-keto.json`** を追加した。取り込み後の店名は **ステーキ（汎用）**、メニュー `group` は **150g / 200g / 300g / 400g** と調味料。付け合わせ野菜（コーン・にんじん・ブロッコリー）を標準成分表ベースの推定量で差し引き **肉のみに近い目安** とした（チェンジブロッコリー前提の換算は廃止）。`notes` 先頭は **【肉のみ目安】** 等（[#11](https://github.com/kzkski/ketolog/issues/11)）。
