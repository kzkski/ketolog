...
- **Today / 設定**: PFC 目標プリセットを最大5スロットの可変運用に拡張（Web / Mobile）。既定の3つに加え、設定からセットを追加・削除できる。Today ヘッダーは使用中のセットのみ横並び表示（#356）。
  - **注意**: 古い Mobile アプリで設定を保存すると、Web で追加したスロット4・5が消えることがあります。Mobile も最新版に更新してください。
...
- **Today / 記録編集**: グラム編集（`½`＋入力）をカートと同様に右寄せに揃え、Web / iOS の統一感を改善した。
...
- **Today / お気に入り検索**: お気に入りタブの検索欄に「全店舗を横断して検索」を追加。ON のとき検索クエリで全店舗のメニューを横断検索し、店舗名ごとに結果を表示する（店舗タブの検索は従来どおりその店のみ）。

## [1.66.1] - 2026-07-10

### Fixed

- **DB / マイグレーション運用**: 本番 `schema_migrations` にだけ記録されていた版（手動適用由来）に対応する履歴同期用プレースホルダーを追加し、`prod-db-migrate` の `supabase db push` が失敗する問題を修正した。あわせて Claude 連携テーブルのマイグレーションを `IF NOT EXISTS` で冪等化し、先行手動適用済みの本番にも安全に適用できるようにした。

## [1.66.0] - 2026-07-10

### Added

- **Web / 設定**: Claude 連携用の Supabase refresh トークンを設定から発行・一覧・失効できる。トークンは発行時に一度だけ表示し、PostgREST（RLS 準拠）経由で食事ログ等にアクセスする用途向け。
- **Mobile / 設定**: Web と同じ「Claude連携」セクションを追加。初版では発行・失効は Web 版の設定から行う案内を表示する。
