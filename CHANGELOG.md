...
- メニュー追加ドロワーで、いま選ばれているお店タブがメニュー登録の宛先になることを冒頭と登録ボタンで明示した。
- 上記ドロワー周りの補足文言で「マイフード」に寄せず、どのお店タブでも同じ読みになるよう「メニュー一覧」などに統一した。
- 食事タブ行の右端に「＋」を1つだけ置き、いま選んでいる食事区分で記録ドロワーを開くようにした（区分ごとの「＋」は廃止）。
- 上記「＋」をエメラルド系の背景で強調し、「この日の記録」トグルを右寄せにして操作導線を揃えた。
- カートパネルの色を、選択中の食事区分タブと同系色にし、展開時に「記録する食事」を切り替えられるようにした（上部タブの区分とも同期）。
