...
- **プリセット**: ステーキチェーン公式栄養表に基づく外食プリセット **`external-steak-keto.json`** を追加した。取り込み後の店名は **ステーキ（汎用）**、メニュー `group` は **150g / 200g / 300g / 400g** と調味料。各 `notes` 先頭に **【チェンブロ前提】** 等のラベルを付け、チェンジブロッコリー換算の有無が一覧で分かるようにした（[#11](https://github.com/kzkski/ketolog/issues/11)）。
