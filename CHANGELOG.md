...
- **プリセット**: ステーキチェーン公式栄養表に基づく外食プリセット **`external-steak-keto.json`** を追加した。取り込み後の店名は **ステーキ（汎用）**、`group` は **品目名**（ワイルドステーキ等）。450g 行の追加（外挿含む）、ワイルドコンボ3種・グリルチキン・乱切り全g、ヒレカットの拡充。ブレードミート・丼類は除外。付け合わせ野菜の差し引き方針は従来どおり（[#11](https://github.com/kzkski/ketolog/issues/11)）。
...
- **今日**: お店タブの店名を、長押しまたは右クリックのメニューから変更できるようにした（お気に入りグループ名は条件付きで追従。スナップショット専用の店は変更不可）（[#118](https://github.com/kzkski/ketolog/issues/118)）。
