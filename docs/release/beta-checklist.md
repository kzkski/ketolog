...
- §9 は **参考（非ブロッキング）**。ベータ公開判定の必須条件には含めない。
...
v2-2（連携ベータ）で JSON エクスポートを分析に使う前提として、**連携フォーマットの契約を先に固定**する。実装詳細は別 Issue で進めるが、ベータ判定では少なくとも次を確認する。

- **契約バージョン**: エクスポート payload に `contractVersion` を含める（例: `1.0`）。
- **スキーマ定義**: JSON Schema などで、必須項目・型・単位（P/F/C の g 単位、日時のタイムゾーン）を明文化する。
- **互換ポリシー**: `minor = 後方互換の追加`、`major = 破壊的変更` を運用ルールとして明記する。
- **検証用サンプル**: 正常系/欠損系/境界値系のサンプル JSON を準備し、取り込み検証に使う。
- **CI 検証**: エクスポート JSON が契約に準拠することを CI で自動チェックする。

**Tracking**
- Status: Not started
- Track: v2-2
- Tracking Issue: [#248](https://github.com/kzkski/ketolog/issues/248)
- Owner: TBD
- DoD: v2-2 連携ベータで利用する JSON 連携契約（`contractVersion`/schema/互換ポリシー）が文書化され、検証手順とサンプルデータの所在が示されている。
