...
- **監視間隔・失敗判定（運用方針）**:
  ...
  - **連続失敗**: **2 回連続**（約 **10 分**以内に 2 度 `503` / 同一系統の Sentry イベントが続く等）を **障害疑いの目安**とする。**Slack 通知・Sentry アラート最小 5 本・P1/P2・スパム抑制**は [sentry-alerts-slack.md](sentry-alerts-slack.md) に従い Sentry 上で設定する。
