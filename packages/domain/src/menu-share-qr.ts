...
export type MenuShareImportItem = {
  name: string;
...
export type MenuQrPayloadV1 = {
  v: 1;
  kind: "menuItem";
  item: MenuShareImportItem;
};

export function buildMenuQrPayloadJson(item: MenuShareImportItem): string {
...
function validateMenuShareImportItemShape(item: MenuShareImportItem): string | null {
 ...
 */
export function parseMenuSharePayload(
  text: string
): { ok: true; item: MenuShareImportItem } | { ok: false; error: string } {
  let raw: unknown;
  try {
    raw = JSON.parse(text);
  } catch {
  ...
  const it = item as MenuShareImportItem;
  const err = validateMenuShareImportItemShape(it);
