...
function LogEntryRow({
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2 px-2.5 py-1.5 sm:px-4 sm:py-2 border-b border-gray-800/40">
      <span className="flex-1 min-w-0 text-xs sm:text-sm text-white truncate leading-snug">{entry.item_name}</span>
      <span className="text-[11px] sm:text-xs text-gray-400 shrink-0 tabular-nums">{entry.grams}g</span>
      <span className="text-[11px] sm:text-xs text-gray-500 shrink-0 tabular-nums w-[6.5rem] sm:w-28 text-right leading-snug">
        P{fmt(entry.protein_g)} F{fmt(entry.fat_g)} C{fmt(entry.carbs_g)}
      </span>
      <button type="button" onClick={onEdit} className="text-gray-400 hover:text-white text-sm sm:text-xs min-h-8 min-w-8 sm:min-h-0 sm:min-w-0 flex items-center justify-center shrink-0 rounded-md sm:rounded-none active:bg-gray-800/70">✎</button>
      <button type="button" onClick={onDelete} className="text-red-400 hover:text-red-300 text-sm sm:text-xs min-h-8 min-w-8 sm:min-h-0 sm:min-w-0 flex items-center justify-center shrink-0 rounded-md sm:rounded-none active:bg-gray-800/70">✕</button>
    </div>
  );
}
        ...
        {logEntries.length > 0 && (
          <div className="flex-none border-b border-gray-800">
            <button
              ...
              className="w-full flex items-center justify-end gap-2 px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs text-gray-400 hover:text-white transition-colors min-h-8 sm:min-h-0 leading-tight"
            >
            ...
            {showLogEntries && (
              <div className="max-h-52 overflow-y-auto">
                {(["breakfast", "lunch", "dinner", "snack"] as MealType[]).map((mt) => {
                  ...
                  return (
                    <div key={mt}>
                      <p className="px-3 sm:px-4 py-0.5 sm:py-1 text-[11px] sm:text-xs text-gray-500 bg-gray-900/50 leading-tight">{MEAL_LABELS[mt]}</p>
                      {items.map((entry) => (
                        <LogEntryRow
                          key={entry.id}
                          entry={entry}
