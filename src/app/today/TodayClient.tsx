...
function PFCBar({ label, current, target, color }: {
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2">
      ...
      <div className="flex-1 h-2 sm:h-2 bg-gray-800 rounded-full overflow-hidden">
...
function SortableRestaurantTab({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex shrink-0 items-stretch border-b-2 min-h-10 sm:min-h-0 ${
    ...
    >
      ...
      <button
        type="button"
        onClick={onSelect}
        className={`pl-1 pr-3 sm:pl-0.5 sm:pr-2.5 py-2 sm:py-2.5 text-sm font-medium whitespace-nowrap text-left transition-colors touch-manipulation min-w-0 max-w-[12rem] sm:max-w-none truncate ${
...
export default function TodayClient({
  ...
  return (
    <>
      {/* ヘッダー */}
      <header className="flex-none flex items-center gap-2 px-4 py-2 sm:py-3 border-b border-gray-800 pt-[max(0.5rem,env(safe-area-inset-top))]">
        ...
        <button onClick={() => setShowSettings(true)}
          type="button"
          className="shrink-0 text-gray-400 hover:text-white transition-colors text-lg sm:text-lg leading-none min-h-10 min-w-10 sm:min-h-0 sm:min-w-0 flex items-center justify-center rounded-lg sm:rounded-none active:bg-gray-800/60 sm:active:bg-transparent">
          ⚙
        </button>
      </header>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {/* 日付ナビゲーション */}
        <div className="flex-none flex items-center justify-between px-3 sm:px-4 py-1.5 sm:py-2 border-b border-gray-800 bg-gray-900 gap-1 sm:gap-2">
          <button type="button" onClick={() => navigateDate(-1)} disabled={loadingDate}
            className="min-h-10 min-w-10 sm:min-h-8 sm:min-w-8 flex items-center justify-center text-gray-400 hover:text-white disabled:opacity-30 transition-colors text-lg sm:text-lg rounded-lg sm:rounded-none active:bg-gray-800/50 shrink-0">
            ‹
          </button>
          <div className="flex flex-row sm:flex-col flex-wrap items-center justify-center gap-x-2 gap-y-0 sm:gap-1 min-w-0 flex-1 px-0.5">
            ...
            {selectedDate !== today && !loadingDate && (
              <button
                ...
                className="text-[11px] text-emerald-400 hover:text-emerald-300 underline-offset-2 hover:underline shrink-0 py-0.5"
              >
                今日に戻る
              </button>
            )}
        ...
        {/* PFCバー */}
        <div className="flex-none px-4 py-2.5 sm:py-3 bg-gray-900 border-b border-gray-800 space-y-1.5 sm:space-y-1.5">
        ...
        {logEntries.length > 0 && (
          <div className="flex-none border-b border-gray-800">
            <button
              ...
              className="w-full flex items-center justify-end gap-2 px-4 py-2 sm:py-2 text-xs text-gray-400 hover:text-white transition-colors min-h-10 sm:min-h-0"
            >
            ...
            {(Object.keys(MEAL_LABELS) as MealType[]).map((type) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-0 min-h-10 sm:min-h-0 py-2 sm:py-2.5 text-xs font-medium border-b-2 transition-colors text-center touch-manipulation ${
                    active
                      ? `${a.row} ${a.label}`
        ...
        <div className="flex-none flex border-b border-gray-800 overflow-x-auto [scrollbar-gutter:stable] pl-[max(0px,env(safe-area-inset-left))] pr-[max(0px,env(safe-area-inset-right))] items-stretch">
          <button
            ...
            className={`px-3 sm:px-4 py-2 sm:py-2.5 text-sm font-medium whitespace-nowrap shrink-0 border-b-2 transition-colors min-h-10 sm:min-h-0 touch-manipulation ${
              selectedRestaurantIdResolved === FAVORITES_TAB_ID
          ...
          <button
            ...
            className={`px-2.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm font-medium whitespace-nowrap shrink-0 border-b-2 transition-colors min-h-10 sm:min-h-0 touch-manipulation max-w-[9.5rem] sm:max-w-none ${
              selectedRestaurantIdResolved === MEXT_COMPOSITION_TAB_ID
