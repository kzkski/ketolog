...
import { importMenuItemsToRestaurant, importRestaurantData } from "./actions/import-export";
import {
  getFoodLogForExport,
  saveMealToLog,
  updateFoodLogEntry,
...
// ─── エクスポートユーティリティ（ペイロード組み立ては `@ketolog/domain/restaurant-json-v1`） ───
...
function downloadRestaurantJson(restaurant: Restaurant, menuItems: MenuItem[]) {
  const payload = buildRestaurantExportDocument(
    ...
    menuItems.map((m) => ({
      ...
      shared_barcode: m.shared_barcode,
      standard_food_code: m.standard_food_code,
...
function ImportRestaurantDrawer({
  ...
  const [parsed, setParsed]         = useState<SingleRestaurantJsonPayload | null>(null);
